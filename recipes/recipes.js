...
const recipeTemplate = recipe => `
  <article class="recipe-card">
    <img src="${recipe.image}" alt="${recipe.name} photo" class="recipe-img">
    <div class="recipe-body">
      ...
      <details>
        ...
        <ul class="ingredients">${recipe.recipeIngredient.map(i => `<li>${i}</li>`).join('')}</ul>
        <ol class="instructions">${recipe.recipeInstructions.map(i => `<li>${i}</li>`).join('')}</ol>
        <div class="meta">
          <span>Author: ${recipe.author}</span>
          ${recipe.datePublished ? `<span> | Published: ${recipe.datePublished}</span>` : ""}
          <span> | ${recipe.prepTime ? `Prep: ${recipe.prepTime}` : ""} ${recipe.cookTime ? `Cook: ${recipe.cookTime}` : ""}</span>
          <span> | Yield: ${recipe.recipeYield || '-'}</span>
        </div>
        ${recipe.url ? `<a class="source" href="${recipe.url}" target="_blank" rel="noopener">View Source</a>` : ""}
      </details>
    </div>
  </article>
`;
...
function renderRecipes(recipeArr) {
  ...
  if (!recipeArr.length) {
    list.innerHTML = `<p class="no-result">No recipes found.</p>`;
    return;
  }
  list.innerHTML = recipeArr.map(recipeTemplate).join('');
}
...
function handleSearch(e) {
  ...
  let result = recipes.filter(r =>
    ...
    r.recipeIngredient.join(' ').toLowerCase().includes(q)
  );
...
function toggleDescriptions() {
  const cards = document.querySelectorAll('.recipe-card .description');
  if (window.innerWidth < 600) {
    cards.forEach(d => d.style.display = "none");
  } else {
    cards.forEach(d => d.style.display = "");
  }
}
