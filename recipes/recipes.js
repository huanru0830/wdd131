...
function renderRecipes(recipesArr) {
  ...
  list.innerHTML = recipesArr.map(r => `
    <article class="recipe-card">
      <img src="${r.image}" alt="${r.name} photo" class="recipe-img">
      <div class="recipe-body">
        ...
        <details>
          ...
          <ul class="ingredients">
            ${r.recipeIngredient.map(i => `<li>${i}</li>`).join('')}
          </ul>
          <ol class="instructions">
            ${r.recipeInstructions.map(i => `<li>${i}</li>`).join('')}
          </ol>
          <div class="meta">
            <span>Author: ${r.author}</span> 
            ${r.datePublished ? `<span> | Published: ${r.datePublished}</span>` : ""}
            <span> | ${r.prepTime ? `Prep: ${r.prepTime}` : ""} ${r.cookTime ? `Cook: ${r.cookTime}` : ""}</span>
            <span> | Yield: ${r.recipeYield || '-'}</span>
          </div>
          ${r.url ? `<a class="source" href="${r.url}" target="_blank" rel="noopener">View Source</a>` : ""}
        </details>
      </div>
    </article>
  `).join('');
...
// Show/hide description on mobile/desktop
